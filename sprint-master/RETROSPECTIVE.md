# Sprint Retrospective

## Data

20/06/2026

## Participantes

- Fabio Ara
- Lucas Toffetti

## Técnica usada

Start, Stop, Continue.

## Start — O que devemos começar a fazer

- Criar backlog antes de começar a programar.
- Registrar decisões no repositório desde o primeiro dia.
- Fazer testes manuais com checklist.
- Reduzir o escopo quando o prazo for curto.

## Stop — O que devemos parar de fazer

- Assumir que haverá tempo para funcionalidades extras.
- Deixar documentação para o último dia.
- Trabalhar sem definição clara de pronto.
- Depender de integrantes que não estarão disponíveis.

## Continue — O que devemos continuar fazendo

- Usar commits pequenos e descritivos.
- Dividir responsabilidades.
- - Manter comunicação objetiva.

## Ações de melhoria

1. Em uma próxima sprint, criar issues no GitHub para cada tarefa do backlog.
2. Fazer uma primeira versão funcional ainda no primeiro dia.
3. Definir horário fixo para daily.
4. Registrar impedimentos assim que aparecerem.

## Observação sobre a equipe

A equipe nominal tinha três integrantes, mas a execução efetiva contou com dois participantes. Isso reduziu a capacidade da sprint e exigiu corte de escopo.
